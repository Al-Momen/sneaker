...
if (!function_exists('isBreadcrumbRoute')) {
    function isBreadcrumbRoute(): bool
    {
        // Fixed route names
        $guestRoutes = [
            'user.login',
        ...
        // Fixed URL substrings or patterns to match
        $guestUrlPatterns = [
            '/faq',
        ];
        ...
        foreach ($guestUrlPatterns as $pattern) {
...
function isWishlist($product = null)
{
    if (!auth()->check()) {
        return '<img class="d-block" src="' .  asset('assets/images/frontend/icon/fav-w.png')  . '" alt="' . __('image') . '">';
    }

    $userId = auth()->id();

    if ($product && $product->wishlists->where('user_id', $userId)->where('product_id', $product->id)->isNotEmpty()) {
        return '<img class="d-block" src="' . asset('assets/images/frontend/icon/fav-w-fill.png') . '" alt="' . __('image') . '">';
    }
...
function discountPrice($mainPrice, $discountPercent = null)
{
    if ($discountPercent && $discountPercent > 0) {
        $discountAmount = ($mainPrice * $discountPercent) / 100;
        $finalPrice = $mainPrice - $discountAmount;
        return showAmount($finalPrice, 2);
    } else {
        return showAmount($mainPrice);
    }
}
