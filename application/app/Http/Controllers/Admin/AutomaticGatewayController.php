...
class AutomaticGatewayController extends Controller
{
    public function index(Request $request, $status = 'all')
    {
        ...
        $query = Gateway::automatic()->with('currencies')->latest();
        ...
        switch ($status) {
            ...
            case '0':
                ...
                break;
            case 'all':
                $query->whereIn('status', [Status::DISABLE, Status::ENABLE]);
                break;
            default:
                break;
        }
        ...
        if ($request->ajax()) {
            return response()->json([
                'html' => view('Admin::components.tables.automatic_data', compact('items'))->render(),
                'pagination' => $items->hasPages() ? view('Admin::components.pagination', compact('items'))->render() : '',
            ]);
        }

        $pageTitle = 'Automatic Gateways';
        return view('Admin::gateways.automatic.list', compact('pageTitle', 'items'));
    }
    ...
    public function update(Request $request, $code)
    {
        ...
        if($request->hasFile('image')) {
            ...
            $gateway->image =fileUploader($request->image,getFilePath('paymentGateway'), getFileSize('paymentGateway'), $gateway->image);
        }
        ...
        $notify[] = ['success', $gateway->name . ' has been updated successfully'];
