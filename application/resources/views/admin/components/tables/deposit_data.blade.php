            ...
            <div class="d-flex justify-content-between justify-content-lg-start gap-3">
                ...
                <div class="user--info d-flex gap-3 flex-shrink-0 align-items-center flex-wrap flex-md-nowrap">
                    ...
                    <div class="user--content">
                        <a class="text-start" href="{{ appendQuery('search', $deposit?->user?->username) }}">
                            {{ __($deposit->user?->fullname ?? 'N/A') }}
                        </a>
                        <br>
                        <a href="{{ route('admin.users.detail', $deposit?->user_id) }}" class="text-start" href="{{ appendQuery('search', $deposit?->user?->username) }}">{{ '@'.__($deposit->user?->username ?? 'N/A') }}</a>
                    </div>
                </div>
            </div>
        </td>
