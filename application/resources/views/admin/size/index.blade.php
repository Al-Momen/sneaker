            ...
            <div class="d-flex flex-wrap justify-content-start">
                <form class="form-inline">
                    <div class="search-input--wrap position-relative">
                        <input type="text" name="search" class="form-control" placeholder="@lang('Search size')..."
                    ...
                    <div class="table-responsive--sm table-responsive">
                        <table class="table table--light style--two custom-data-table">
                            ...
                            <tbody id="items_table__body">
                                @include('admin.components.tables.size_data')
                            </tbody>
                        </table>
                    </div>
                </div>

                <div id="pagination-wrapper" class="pagination__wrapper py-4 {{ $sizes->hasPages() ? '' : 'd-none' }}">
                    @if ($sizes->hasPages())
                        {{ paginateLinks($sizes) }}
                    @endif
                </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"> @lang('Add Size')</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('admin.size.store') }}" class="edit-route" method="POST"
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    <label>@lang('Size')</label>
                                    <div class="col-sm-12">
                                        <input type="number" class="form-control" value="{{ old('size') }}"
                                            name="size" required>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-12">
                                <div class="form-group mt-3">
                                    <label class="fw-bold">@lang('Status')</label>
                                    <label class="switch m-0">
                                        <input type="checkbox" class="toggle-switch" name="sizeStatus">
                                        <span class="slider round"></span>
                                    </label>
                                </div>
                            </div>
    ...
    <script>
        (function($) {
            ...
            $(document).on('click', '.editCatBtn', function() {
                ...
                modal.find('input[name="size"]').val($(this).data('size'));

                if ($(this).data('status') == 1) {
                    modal.find('input[name="sizeStatus"]').prop('checked', true);
                }

                if ($(this).data('status') == 0) {
                    modal.find('input[name="sizeStatus"]').prop('checked', false);
                }
                modal.modal('show');
            });

            let baseUrl = `{{ route('admin.size.index', ':status') }}`;
            ...
            $('#status-filter').on('change', function() {
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                        $('#items_table__body').html('<tr><td colspan="4" class="text-center">Loading...</td></tr>');
                    },
