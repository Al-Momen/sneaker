                                        ...
                                        <td>
                                            <div class="d-flex justify-content-end flex-wrap gap-1">
                                                <button class="btn btn-outline--primary btn-sm editBtn"
                                                    ...
                                                    data-key="{{ $key }}" title="Configure"><i
                                                        class="la la-cogs"></i></button>
                                                <button type="button" data-type="{{ $key }}"
                                                    class="btn btn-sm btn-outline--dark helpBtn" title="Help">
                                                ...
                                                @if ($credential->status == 1)
                                                    <button class="btn btn-outline--danger btn-sm confirmationBtn"
                                                        ...
                                                        title="Disabled"><i class="las la-ban"></i></button>
                                                @else
                                                    <button class="btn btn-outline--success btn-sm confirmationBtn"
                                                        ...
                                                        data-action="{{ route('admin.setting.socialite.credentials.status.update', $key) }}"
                                                        title="Enabled"><i class="las la-check"></i></button>
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
    ...
    <script>
        (function($) {
            ...
            $(document).on('click', '.editBtn', function() {
                ...
                let updateRoute = "{{ route('admin.setting.socialite.credentials.update', ':key') }}";
                let callbackRoute = "{{ route('user.social.login.callback', ':key') }}";
                updateRoute = updateRoute.replace(':key', data.key);
                callbackRoute = callbackRoute.replace(':key', data.key);
                ...
                modal.find('.callback').val(callbackRoute);
                modal.modal('show');
            });
